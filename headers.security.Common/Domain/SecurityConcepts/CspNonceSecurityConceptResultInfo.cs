...
public class CspNonceSecurityConceptResultInfo : ISecurityConceptResultInfo
{
    ...
    public static CspNonceSecurityConceptResultInfo Reuse(HashSet<string> nonces)
    {
        ...
        return new()
        {
            Message = $"The following {noncePart} to be re-used: {{}}. A nonce that is re-used can be predicted by an attacker and renders the CSP useless.",
    ...
    public static CspNonceSecurityConceptResultInfo LowComplexity(string nonce) =>
        new()
        {
            Message = "The nonce {} is of lower than recommended complexity, nonces should be at least 16 characters long.",
            FormatTokens = [[nonce]]
        };
}